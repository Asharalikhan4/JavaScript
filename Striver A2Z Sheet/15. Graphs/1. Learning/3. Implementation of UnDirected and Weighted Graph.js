class UndirectedWeightedGraph {
  ...
  addEdge(source, destination, weight) {
    ...
    const destinationEdges = this.adjacencyList.get(destination);
    if (!destinationEdges.find((edge) => edge.node === source)) {
      destinationEdges.push({ node: source, weight: weight });
    }
  }

  print() {
    for (const [vertex, edges] in this.adjacencyList) {
